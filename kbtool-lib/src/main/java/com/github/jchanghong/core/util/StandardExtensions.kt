...
inline fun <R> Boolean.letIfTrue(ifBlock: () -> R, elseBlock: () -> R): R =
    letIf({ this }, { run(ifBlock) }, { run(elseBlock) })

inline fun <T> T.letIf(condition: T.() -> Boolean, ifBlock: (T) -> Unit): Unit = letIf(condition, ifBlock) {}

inline fun <T, R> T.letIf(condition: T.() -> Boolean, ifBlock: (T) -> R, elseBlock: (T) -> R): R =
    if (run(condition)) run(ifBlock) else run(elseBlock)
...
inline fun Boolean.alsoIfTrue(ifBlock: () -> Unit, elseBlock: () -> Unit): Boolean =
    alsoIf({ this }, { run(ifBlock) }, { run(elseBlock) })

inline fun <T> T.alsoIf(condition: T.() -> Boolean, ifBlock: (T) -> Unit): T = alsoIf(condition, ifBlock) {}

inline fun <T, R> T.alsoIf(condition: T.() -> Boolean, ifBlock: (T) -> R, elseBlock: (T) -> R): T =
    also { if (run(condition)) run(ifBlock) else run(elseBlock) }
...
inline infix fun <T> (() -> T).andThrow(crossinline throwable: T.() -> Throwable): () -> Nothing =
    andThen { throw throwable() }
...
inline fun <T> T.print(outStream: PrintStream = System.out, noinline transform: (String) -> String = { "$it = " }): T =
...
inline fun <T1, T2, T3, R> ((T1, T2, T3) -> R).with(param1: T1, param2: T2, param3: T3): () -> R =
    { this(param1, param2, param3) }

inline fun <T1, T2, T3, T4, R> ((T1, T2, T3, T4) -> R).with(param1: T1, param2: T2, param3: T3, param4: T4): () -> R =
    { this(param1, param2, param3, param4) }

inline fun <T1, T2, T3, T4, T5, R> ((T1, T2, T3, T4, T5) -> R).with(
    param1: T1,
    param2: T2,
    param3: T3,
    param4: T4,
    param5: T5
): () -> R = { this(param1, param2, param3, param4, param5) }

inline fun <T1, T2, T3, T4, T5, T6, R> ((T1, T2, T3, T4, T5, T6) -> R).with(
    param1: T1,
    param2: T2,
    param3: T3,
    param4: T4,
    param5: T5,
    param6: T6
): () -> R = { this(param1, param2, param3, param4, param5, param6) }
...
inline fun <T, R> ((T, T, T, T) -> R).collapseParams(): (Array<T>) -> R =
    { a -> a.requireSize(4); this(a[0], a[1], a[2], a[3]) }

inline fun <T, R> ((T, T, T, T, T) -> R).collapseParams(): (Array<T>) -> R =
    { a -> a.requireSize(5); this(a[0], a[1], a[2], a[3], a[4]) }
